...
function logAttempt(subject,title,c,n){
  if(!n)return;state.attempts=state.attempts||[];
  state.attempts.push({d:state.today,s:subject,t:title,c:c,n:n});
...
function gachaRoll(){
  ...
  if(state.gachaCount%3===0)return 100;        // 3回に1回は100pt（最大）
  const off=[20,25,30,35];return off[Math.floor(Math.random()*off.length)];
}
function openGacha(){
  if(state.gachaLog[state.today])return; // 1日1回まで
  gachaPending={points:gachaRoll()};
...
function csAnswerQ(qi,ci){
  ...
  if(c.ok){
    ...
    if(first){recordStat(csState.cfg.statKey,false);markReview(csState.cfg.statKey,csState.cfg.setIdx);csState.ftTotal++;}
...
function kokuAnswer(qi,ci){
  ...
  if(c.ok){
    ...
    if(first){recordKokugo(false);markReview('kokugo',kokuState.setIdx);kokuState.ftTotal++;}
    btns[ci].classList.add('wrong');btns[ci].disabled=true;
...
function awardBonus(pts){
  state.points=(state.points||0)+pts;state.totalEarned=(state.totalEarned||0)+pts;
  state.badges=state.badges||[];var nb=null;
  MILESTONES.forEach(function(m){if(state.totalEarned>=m.pt&&state.badges.indexOf(m.id)<0){state.badges.push(m.id);nb=m;}});
  save();try{renderTop();}catch(e){}return nb;
}
function goHakken(){var t=document.querySelector('.tab[data-tab="hakken"]');if(t)t.click();}
...
function pickMission(){
  ...
  if(state.missionDay&&state.missionDay.date===state.today&&pool[state.missionDay.idx])return state.missionDay;
  ...
  state.missionDay={date:state.today,idx:idx};save();return state.missionDay;
}
function renderMado(){
  ...
  box.innerHTML='<div class="mado-card">'+inner+'<button class="btn btn-ghost btn-sm" id="madoSeen" style="margin-top:11px;width:auto"'+(seen?' disabled':'')+'>'+(seen?'✓ +10pt ゲットずみ':'🔖 よんだ！(+10pt)')+'</button></div>';
  const b=document.getElementById('madoSeen');
  if(b&&!seen)b.addEventListener('click',function(){state.mado.seen=true;state.madoSeen=(state.madoSeen||0)+1;awardBonus(10);if(typeof celebrate==='function')celebrate();renderMado();});
}
function renderMission(){
  ...
  box.innerHTML='<div class="mission-card"><div class="mission-m">🧭 '+rubyize(it.m)+'</div><div class="mission-hint">ヒント：'+rubyize(it.hint)+'</div><textarea id="missionInput" class="answer-area" placeholder="やってみて、気づいたこと・わかったことを書こう"></textarea><button class="btn btn-green btn-sm" id="missionSave" style="margin-top:8px;width:auto">✅ やってみた！記録する (+10pt)</button><div id="missionMsg" class="sec-sub" style="min-height:16px;margin-top:6px"></div></div>';
  document.getElementById('missionSave').addEventListener('click',function(){
    ...
    var bonus='';
    if(state.missionBonusDate!==state.today){awardBonus(10);state.missionBonusDate=state.today;save();bonus=' (+10pt🎉)';}
    document.getElementById('missionMsg').textContent='記録したよ！「メモ」タブにのこります。よくがんばったね⚾'+bonus;
...
function renderHakken(){
  ...
  if(sv&&!sv.dataset.wired){sv.dataset.wired='1';sv.addEventListener('click',function(){
    ...
    addLog('はっけん','今日のはっけん',v,'');state.hakkenCount=(state.hakkenCount||0)+1;
    var hb='';if(state.hakkenBonusDate!==state.today){awardBonus(10);state.hakkenBonusDate=state.today;hb=' (+10pt🎉)';}
    save();
    ta.value='';document.getElementById('hakkenMsg').textContent='きろくしたよ！その「気づく力」が合格する子の力⚾'+hb;
